...
increaseBtns.forEach((btn, index) => {
  btn.addEventListener('click', () => {
      ...
      quantityInputs[index].value = qty + 1
      updateTotal(index)
  })
})
...
function updateTotal(index) {
  const price = parseFloat(document.querySelectorAll('.price')[index].textContent.replace('$', ''))
  const qty = parseInt(quantityInputs[index].value)
  const total = (price * qty).toFixed(2)
  document.querySelectorAll('.total')[index].textContent = `$${total}`
}
